...
<body>
    ...
    <div class="container mt-4">
        ...
        <ul class="list-group">
            @foreach ($ordertrans as $ot)
            <div class="card user-card">
                <div class="card-body">
                    <h6 class="card-title font-weight-bold">Item Name: {{ $ot->ItemName }}</h6>
                    <h6 class="card-title font-weight-bold">QTY : {{ $ot->qty }}</h6>
                    <!-- Edit and Delete buttons with icons -->
                    <div class="btn-group mt-3">
                        <a type="button" class="btn btn-sm btn-warning" href="/Order/ChangeStatus?route=CompleteOTListPending&OrderTransId={{ $ot->OrderTransId }}&status=1" data-toggle="modal" data-target="#editModal{{$ot->OrderTransId}}">
                            <i class="fas fa-trash">Not Ready</i>
                        </a>
                    </div>
                </div>
            </div>
...
<script>
    ...
    $('.btn-warning').click(function () {
        var result = confirm("Order Ready ");
        if (!result) {
            return false;
        }
